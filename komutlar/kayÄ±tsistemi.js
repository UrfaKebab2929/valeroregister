...
exports.run = async(client, message, args) => {
  ...
  const discordjstürkiye = new Discord.RichEmbed()
  ...
  .addField(prefix + "kayıt-isim-ayarla (uye) (yas)", "Kayıt olan üyelerin isimlerini ayarlama şekli. `Örn` Gürkan | 19 \n`(uye)` isim, `(yas)` ise yaşı ifade eder. (Yaş eklemek zorunda değilsiniz)")
  ...
  .setTimestamp()
  message.channel.send(discordjstürkiye)
};

exports.conf = {
  enabled: true,
  guildOnly: false,
  aliases: ['kayitsistemi', 'kayıt-sistemi','yardım'],
  permLevel: 0
};
